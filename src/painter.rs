...
use miniquad::{
    Bindings, BlendFactor, BlendState, BlendValue, BufferId, BufferLayout, BufferType, Context,
    Equation, GlContext, Pipeline, PipelineParams, ShaderId, VertexAttribute, VertexFormat,
};
...
impl Painter {
    pub fn new(ctx: &mut Context) -> Painter {
        ...
        let bindings = Bindings {
            ...
            index_buffer,
            images: vec![ctx.new_render_texture(miniquad::TextureParams {
                ..Default::default()
            })],
        };
    ...
    pub fn free_texture(&mut self, ctx: &mut Context, tex_id: egui::TextureId) {
    ...
    pub fn paint(
        ...
        ctx.apply_uniforms(miniquad::UniformsSource::table(&screen_size_in_points));
        ...
        for mesh in meshes {
            ...
            if ctx.buffer_size(self.bindings.vertex_buffers[0]) < vertices_size_bytes {
                ...
                self.bindings.vertex_buffers[0] = ctx.new_buffer(
                    BufferType::VertexBuffer,
                    miniquad::BufferUsage::Stream,
                    miniquad::BufferSource::empty::<usize>(vertices_size_bytes),
                );
            }
            ctx.buffer_update(
                self.bindings.vertex_buffers[0],
                miniquad::BufferSource::slice(&mesh.vertices),
            );
            ...
            if ctx.buffer_size(self.bindings.index_buffer) < indices_size_bytes {
                ...
                self.bindings.index_buffer = ctx.new_buffer(
                    BufferType::IndexBuffer,
                    miniquad::BufferUsage::Stream,
                    miniquad::BufferSource::empty::<usize>(indices_size_bytes),
                );
            }
            ctx.buffer_update(
                self.bindings.index_buffer,
                miniquad::BufferSource::slice(&mesh.indices),
            );
            ...
            self.bindings.images[0] = match mesh.texture_id {
                ...
                egui::TextureId::User(id) => unsafe {
                    miniquad::TextureId::from_raw_id(miniquad::RawId::OpenGl(id as u32))
                },
            };
